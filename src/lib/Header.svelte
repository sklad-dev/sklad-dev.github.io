<header class="sticky top-0 z-50 w-full bg-white">
  <div class="flex justify-between items-center py-4 px-6">
    <div class="flex items-center">
      <img src="./src/assets/logo.png" alt="Sklad Logo" class="w-12">
      <div class="flex items-center">
        <p class="text-xl md:text-2xl font-bold font-mono">Sklad.</p>
        <span class="inline-flex items-center rounded-xl bg-blue-50 px-2 py-1 text-xs font-medium text-blue-700 inset-ring inset-ring-blue-700/10">prototype</span>
      </div>
    </div>
    <a href="https://github.com/sklad-dev/Sklad" target="_blank" class="transition-opacity hover:opacity-70 focus-visible:ring-2 focus-visible:ring-indigo-500 rounded-full">
      <img src="./src/assets/github-logo.png" alt="GitHub" class="h-9">
    </a>
  </div>
</header>
